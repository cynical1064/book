				...
				$.get(''+options.xmlUrl+'', function(d){
					$(d).find('event').each(function(){
						...
						function multidaylist(){
							$('#'+uniqueId+' .monthly-list-item[data-number="'+i+'"]').addClass('item-has-event').append('<a href="'+eventURL+'" class="listed-event"  data-eventid="'+ eventId +'" style="background:'+eventColor+'" title="'+eventTitle+'"><div class="roomName">'+roomname+'</div><div>'+eventTitle+'</div></a>');
						}
		...
		$(document.body).on('click', '#'+uniqueId+' .listed-event', function (e) {
			...
			if(selectRoom == '예약가능'){
				...
				if(chkInTarget==''){
					...
					alert('체크인 날짜를 체크해주세요');
					
				}else if(chkOutTarget == ''){
					
					alert('체크아웃 날짜를 체크해주세요');
					
				}else{
					
					var href = $(this).attr('href', 'dateTest.jsp?chkInTarget='+chkInTarget+'&chkOutTarget='+chkOutTarget+'&roomName='+roomname);
					
				}
				
			}else{
				
				alert('예약 가능한 방을 선택해주세요');
